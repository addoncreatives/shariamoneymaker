...
# Din ønskede delsektor-struktur som sikkerhedsnet / fallback
TARGET_SUBSECTORS = [
...
# =====================================================================
#  GOOGLE SHEETS / EXCEL AGENT (OPDATERET)
# =====================================================================
class GoogleSheetsAgent:
    ...
    def get_current_weights_and_sectors(self) -> tuple:
        ...
        try:
            ...
            # Hent dine opdaterede strategiske delsektorer fra Opsummering, hvis de findes
            strategic_sectors = self.get_strategic_sectors()
            ...
            sector_distribution = {s: 0.0 for s in strategic_sectors}
            ...
            for _, row in df.iterrows():
                ...
                for target_s in strategic_sectors:
    ...
    def get_strategic_sectors(self) -> list:
        """
        Henter dine 21 strategiske delsektorer dynamisk fra fanen 'Opsummering' (Kolonne J: Sektorer).
        Hvis den fejler eller er tom, falder den fejlfrit tilbage til TARGET_SUBSECTORS.
        """
        try:
            raw_df = self._read_tab_as_df("Opsummering")
            sektor_col = self._find_column_by_keyword(raw_df, "sektorer") or self._find_column_by_keyword(raw_df, "sektor")
            
            if not sektor_col:
                return TARGET_SUBSECTORS
                
            sectors = []
            for val in raw_df[sektor_col]:
                if pd.isna(val):
                    continue
                val_str = str(val).strip()
                # Filtrer overskrifter og støj fra
                if val_str and len(val_str) < 40 and val_str.lower() not in ["sektorer", "sektorere", "sektor", "status"]:
                    sectors.append(val_str)
                    
            return sectors if len(sectors) > 5 else TARGET_SUBSECTORS
        except Exception:
            return TARGET_SUBSECTORS
    ...
    def get_current_holdings_details(self) -> list:
        try:
            ...
            print(f"Fejl under hentning af positioner: {str(e)}")
            return []
...
# =====================================================================
#  SCREENER & COMPLIANCE AGENT (MAPPING AF DINE DELSEKTORER)
# =====================================================================
...
# =====================================================================
#  COUNCIL AGENT (GEMINI 3.5 FLASH - MED CASCADE FALLBACK)
# =====================================================================
class CouncilAgent:
    ...
    def run_proactive_analysis(self, candidates_data: list, category: str, deficit: float, current_portfolio_str: str, current_holdings_str: str, sector_distribution_str: str) -> str:
        ...
        # CASCADE FALLBACK SYSTEM (Sikrer mod 503-fejl)
        ...
        for model_name in models_to_try:
            ...
            print(f"Forsøger at kalde Gemini API med model: {model_name}...")
            try:
                ...
                if 'candidates' in data and len(data['candidates']) > 0:
                    ...
                    print(f"Succesfuld generering med model: {model_name}")
                    return raw_html.replace("```html", "").replace("```", "").strip()
            except Exception as e:
                print(f"Advarsel: Fejl på {model_name}: {str(e)}. Prøver næste i rækken...")
                time.sleep(1)

        raise RuntimeError("Alle Gemini-modeller i systemet er midlertidigt utilgængelige (503-fejl).")


# =====================================================================
#  PODCAST AGENT (MED CASCADE FALLBACK & FREE TTS)
# =====================================================================
class PodcastAgent:
    def __init__(self, api_key: str):
        self.api_key = api_key
    ...
    def generate_script(self, report_html: str) -> list:
        prompt = f"""
        Du er en erfaren podcast-producer. Omskriv denne HTML-investeringsrapport til et skarpt, utroligt medrivende og naturligt samtalemanuskript på DANSK:
        {report_html}
        ...
        for model_name in models_to_try:
            url = f"https://generativelanguage.googleapis.com/v1beta/models/{model_name}:generateContent?key={self.api_key}"
            print(f"Forsøger podcast-manuskript med: {model_name}...")
            try:
                ...
                print(f"Fejl på {model_name}: {str(e)}")
                time.sleep(1)
        return []
...
class DeliveryAgent:
    ...
    def send_email(subject: str, html_content: str, attachment_path: str = None):
        ...
        try:
            ...
            server.quit()
            print("Succes: Rapport og podcast-afspilning sendt.")
        except Exception as e:
...
def main():
    try:
        print("Opstarter LLM Council 4x25% med delsektorer, fejlsikring og Podcast...")
        ...
        if GEMINI_API_KEY:
            ...
            # Kalder med automatisk fallback-rækkefølge mod 503-fejl
            ...
            # 9. Generer lyd-podcast (med fejlsikring)
